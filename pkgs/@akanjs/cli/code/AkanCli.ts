...
import { stripAnsi } from "../application/devLogBuffer";
...
export class AkanCli {
  ...
  /**
   * Bundling rewrites `import.meta.dir` to the output directory, where the entry is `index.js` beside this
   * module; from source it is one level up and still TypeScript. `Bun.main` is the last resort and is only
   * right when the CLI is the process — an SDK embedder's main is its own.
   */
  static #resolveEntry() {
    const candidates = [path.join(import.meta.dir, "index.js"), path.join(import.meta.dir, "..", "index.ts")];
