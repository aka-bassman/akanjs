...
/**
 * Opt-in MCP exposure for one endpoint or slice. Absent means invisible to agents, and that default is the
 * security boundary: a guard stops the call, but the name and argument schema of something like
 * `getAccessTokenByAdmin` are themselves worth hiding, and `tools/list` publishes both.
 */
export interface McpOption {
  expose: boolean;
  /** Also publish an addressable `akan://` template beside the tool. Query endpoints only. */
  resource?: boolean;
  /** Overrides for the hints derived from the endpoint type and key. Clients treat these as untrusted. */
  readOnly?: boolean;
  destructive?: boolean;
  idempotent?: boolean;
}

/** MCP exposure for the CRUD endpoints a slice generates. Each verb opts in on its own, like its guards do. */
export interface McpSliceOption {
  get?: boolean;
  create?: boolean;
  update?: boolean;
  remove?: boolean;
  /**
   * The model's own unfiltered list and insight. It sits here rather than on a slice option because the `""`
   * slice is generated by `slice()` itself and so has nowhere for an author to write `mcp: { expose: true }`.
   */
  list?: boolean;
}
...
export interface SerializedSignal {
  ...
  /** Exposure of the CRUD endpoints generated from this model's guards, which carry no signalOption of their own. */
  mcp?: McpSliceOption;
  getGuards?: string[];
  cruGuards?: string[];
  createGuards?: string[];
