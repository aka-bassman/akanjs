  ...
  const cols = useMemo(() => {
    ...
    return [
      ...
      ...(actions
        ? [
            {
              ...
              render: (_: any, model: Light, idx: number) => (
                <div className="flex gap-1">
                  {(typeof actions === "function" ? actions(model, idx) : actions)
                    .filter((action) => typeof action !== "string")
                    .map((action, idx) => (
                      <Action key={`${model.id}-${idx}`} model={model} action={action} slice={slice} />
                    ))}
                </div>
              ),
            },
        ...
        : []),
    ] as { key: string; dataIndex: string; title: string; render: (...args: any[]) => ReactNode }[];
  }, []);
  return (
    <div className={className}>
      ...
      {!modelListLoading
        ? modelList.map((model) => (
            <div key={model.id}>
              <Model.EditModal id={model.id} slice={slice} renderTitle={renderTitle}>
                <RenderTemplate id={model.id} />
              </Model.EditModal>
              {renderView ? (
                <Model.ViewModal id={model.id} slice={slice} renderTitle={renderTitle} renderView={renderView} />
              ) : null}
            </div>
          ))
        : null}
