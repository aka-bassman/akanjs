...
describe("AkanApp", () => {
  ...
  test("hands the MCP config to the children that actually mount it", async () => {
    // The gateway mounts nothing itself, and `server.ts` is generated with no options — so an object written in
    // `main.ts` reaches the process that serves `/mcp` only if every field has an env spelling on both sides.
    const root = await mkdtemp(path.join(tmpdir(), "akan-app-mcp-env-"));
    tempRoots.push(root);
    const envPath = path.join(root, "mcp-env.json");
    const serverPath = path.join(root, "server.ts");
    const runtimeDir = path.join(root, "runtime");
    const port = 24_000 + Math.floor(Math.random() * 10_000);
          ...
          export const server = {
            async start() {
              await Bun.write(
                ...
                JSON.stringify(Object.fromEntries(Object.entries(process.env).filter(([key]) => key.startsWith("AKAN_MCP")))),
              );
              const http = Bun.serve({
                unix: process.env.AKAN_CHILD_SOCKET,
                fetch() { return new Response("ok"); },
    ...
    const app = new AkanApp(serverPath, {
      ...
      mcp: {
        ...
        pageSize: 25,
        language: "ko",
        auth: { authorizationServers: ["https://auth.example.com"] },
      },
    });
    const running = app.start();
    try {
      const seen = await waitFor(async () => {
        ...
        return (await file.exists()) ? ((await file.json()) as Record<string, string>) : null;
      });
      expect(seen).toMatchObject({
        AKAN_MCP: "true",
        AKAN_MCP_INSTRUCTIONS: "Domain tools for the test app.",
        AKAN_MCP_ALLOWED_ORIGINS: "https://console.example.com",
        AKAN_MCP_PAGE_SIZE: "25",
        AKAN_MCP_LANGUAGE: "ko",
