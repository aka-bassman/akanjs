    ...
    <Screen className="px-5 pb-28">
      <Layout.TopInset className={cn(appNavClass, "flex items-center px-5")} estimatedHeight={pageConfig.topInset}>
        <div className="flex w-full items-center justify-between">
          <div>
            <p className="text-foreground/40 text-xs uppercase tracking-[0.24em]">Messages</p>
      ...
      <div className="space-y-3 pt-5">
        {[
          ...
          <Link className={appCard(undefined, "flex items-center gap-3 rounded-3xl p-4")} href="/inbox/chat" key={name}>
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center justify-between gap-3">
                ...
                <p className="text-foreground/40 text-xs">{time}</p>
              </div>
              <p className="mt-1 truncate text-foreground/50 text-sm">{message}</p>
            </div>
