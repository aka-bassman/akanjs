...
Query updates (`updateOne`, `updateMany`, `removeMany`, `bulkWrite`) compile to a single atomic SQL statement and do not load documents.
...
`updateOne`, `updateMany`, `removeMany`, and `bulkWrite` write directly in the database and do not fire save/update/remove hooks.
