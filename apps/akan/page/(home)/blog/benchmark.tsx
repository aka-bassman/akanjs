          ...
          <div className="mt-5 space-y-8">
            {akanResults.map((item) => (
              <section key={item.scenario}>
                ...
                <p className="mt-1 font-mono text-foreground/45 text-sm">{item.scenario}</p>
                <p className="mt-3 text-foreground/75 leading-7">{l.trans(item.desc)}</p>
                <ul className="mt-3 list-disc space-y-1 pl-5 text-foreground/75 leading-7">
                  <li>
                    {l.trans({
    ...
    <div>
      <div className="flex flex-wrap items-baseline justify-between gap-2">
        <div>
          ...
          <p className="font-mono text-foreground/45 text-sm">{item.scenario}</p>
        </div>
  ...
  return (
    <div>
      <div className="mb-1 flex justify-between gap-3 text-sm">
        ...
        <span className="font-mono text-foreground/55">{value}</span>
      </div>
  ...
  return (
    <div>
      ...
      <div className="h-2 overflow-hidden rounded-full bg-foreground/10">
        <div
          className={`h-full rounded-full ${isAkan ? "bg-primary" : "bg-foreground/50"}`}
