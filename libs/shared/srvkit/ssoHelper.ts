...
export const getOAuthRedirectUrl = (
  ...
  if (!config) throw new Err("shared.error.ssoNotConfigured", { type });
...
export const getSsoCode = (request: Bun.BunRequest) => {
  ...
  if (!code) throw new Err("shared.error.invalidSsoCallbackMissingCode");
  return code;
};

export const getSsoOrigin = (request: Bun.BunRequest) => {
  const origin = request.cookies.get("ssoOrigin");
  if (!origin) throw new Err("shared.error.invalidSsoCallbackMissingOrigin");
  return origin;
};
...
const exchangeCodeForToken = async (
  ...
  if (!data.access_token) throw new Err("shared.error.failedToGetAccessToken", { type });
...
export const assertSsoConfigured = (type: utilOption.SSOType) => {
  if (!ssoRegistry.has(type)) throw new Err("shared.error.ssoNotConfigured", { type });
};
...
export const verifyAppleUser = async (payload: { code: string }, origin: string, sso: AppleCredential) => {
  ...
  if (!tokens.id_token) {
    throw new Err("shared.error.noAppleIdToken");
  }
