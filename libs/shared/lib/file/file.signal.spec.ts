...
export const getFileInputSamples = (num = 1): [fileList: FileList, metas: cnst.FileMeta[]] => {
  ...
  const files = buffers.map((buffer) => new File([buffer], "image.jpg", { type: "image/jpeg" }));
...
export const addAndWaitActiveFiles = async (
  ...
  const activeFiles = await Promise.all(
    files.map(async (file) => {
      ...
      expect(file.url?.length ?? 0).toBeGreaterThan(0);
      expect(file.abstractData?.length ?? 0).toBeGreaterThan(0);
      return file;
    }),
  );
